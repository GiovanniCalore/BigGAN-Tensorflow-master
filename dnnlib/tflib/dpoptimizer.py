...
class DPOptimizer:
    ...
    def __init__(self,
        ...
        tf_optimizer:           str             = "tensorflow_privacy.privacy.optimizers.dp_optimizer.DPAdamGaussianOptimizer", # Underlying optimizer class.
        ...
        share:                  "DPOptimizer"   = None,                     # Share internal state with a previously created optimizer?
        ...
        **kwargs):
        ...
        if share is not None:
            assert isinstance(share, DPOptimizer)
    ...
    def _get_device(self, device_name: str):
        ...
        with tfutil.absolute_name_scope(self.scope + "/Devices"), tf.device(device_name), tf.control_dependencies(None):
            if device_name not in self._shared_optimizers:
                ...
                name=optimizer_name,
                l2_norm_clip = self.l2_norm_clip, #NORMA 1 DOVREBBE ANDARE BENE, CIOE' PRODUCE GRADIENTI SIMILI A COME SE NON CI FOSSE
                noise_multiplier = self._noise_multiplier, # STDEV = l2*noise_multiplier
                num_microbatches = 4,
    ...
    def get_stds(self, loss: TfExpression, trainable_vars: Union[List, dict]) -> None:
        tfutil.assert_tf_initialized()
        ...
        with tf.name_scope(self.id + "_grad"), tf.device(device.name), tf.control_dependencies(deps):
            ...
            #    print(i, ": ", trainable_vars[i])

            #sys.exit()
            loss = self.apply_loss_scaling(tf.cast(loss, tf.float32))
            gate = tf.train.Optimizer.GATE_NONE  # disable gating to reduce memory usage
            stds = device.optimizer.get_gradients(loss=loss, var_list=trainable_vars, gate_gradients=gate, id=self.id)
    ...
    def register_gradients(self, loss: TfExpression, trainable_vars: Union[List, dict]) -> None:
        ...
        # LOSS SHAPE = 4, come il numero di sample per GPU giustamente
        # TRAINABLES LENGTH = 138 per G, 48 per D
        ...
        with tf.name_scope(self.id + "_grad"), tf.device(device.name), tf.control_dependencies(deps):
            ...
            #ciao = device.optimizer.get_norms(loss=loss, var_list=trainable_vars, gate_gradients=gate, id=self.id)
            #print(trainable_vars)
            #sys.exit()
            #stds = device.optimizer.get_gradients(loss=loss, var_list=trainable_vars, gate_gradients=gate, id=self.id)
            grad_list, _ = device.optimizer.compute_gradients(loss=loss, var_list=trainable_vars, gate_gradients=gate, id=self.id) # GRAD_LIST LENGTH = 138 per G, 48 per D
