...
class MetricBase:
    ...
    def _reset(self, network_pkl=None, run_dir=None, data_dir=None, dataset_args=None, mirror_augment=False):
    ...
    def run_old(self, network_pkl, run_dir=None, data_dir=None, dataset_args=None, mirror_augment=None, num_gpus=1, tf_config=None, log_results=True, Gs_kwargs=dict(is_validation=True)):
        ...
        log_file = os.path.join('./results/metric-%s.txt' % self.name)
        with dnnlib.util.Logger(log_file, 'a'):
            print(self.get_result_str().strip())

    def run(self, sess, fake_images_random_normal, interp_images, id, base_dir, results_dir, run_dir=None, data_dir=None, dataset_args=None, mirror_augment=None, num_gpus=1, tf_config=None, log_results=True, Gs_kwargs=dict(is_validation=True)):
        self._reset(run_dir=run_dir, data_dir=data_dir, dataset_args=dataset_args, mirror_augment=mirror_augment)
        time_begin = time.time()
        self._report_progress(0, 1)
        self._evaluate(sess, fake_images_random_normal, interp_images, Gs_kwargs=Gs_kwargs, num_gpus=num_gpus)
        self._report_progress(1, 1)
        self._eval_time = time.time() - time_begin # pylint: disable=attribute-defined-outside-init
        self.id = id

        log_file = os.path.join(base_dir + '%s/metric-%s.txt' % (results_dir ,self.name))
        with dnnlib.util.Logger(log_file, 'a'):
    ...
    def get_result_str(self):
        ...
        result_str = 'BigGAN_256_' + self.id
