    ...
    def _evaluate(self, sess, fake_images_random_normal, interp_images, Gs_kwargs, num_gpus):
        ...
        for gpu_idx in range(num_gpus):
            with tf.device('/gpu:%d' % gpu_idx):
                ...
                # noises
                #latents = tf.random_normal(shape=[16, 1, 1, 128], name='random_z_latents')
                #images = Gs(latents)
                #labels = self._get_random_labels_tf(self.minibatch_per_gpu)
                #images = Gs_clone.get_output_for(latents, labels, **Gs_kwargs)

                images = tflib.convert_images_to_uint8(tf.transpose(fake_images_random_normal, perm=[0, 3, 1, 2])) # CONVERTIRE LE DIMENSIONI
        ...
        for begin in range(0, self.num_images, minibatch_size):
            ...
            activations[begin:end] = np.concatenate(sess.run(result_expr), axis=0)[:end-begin]
            print("FID: " + str(begin), end="\r")
