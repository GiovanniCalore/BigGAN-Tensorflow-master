...
from tensorflow_privacy.privacy import optimizers
...
class BigGAN_256(object):
    ...
    def __init__(self, sess, args):
        ...
        self.id = self.dataset_name.split('/')[-1]
        self.results_dir = args.results_dir
        self.checkpoint_name = args.ckpt
        ...
        self._tf_record_dataset = self.dataset_name.split('/')[-1]
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            #self.d_optim = self.d_opt.minimize(self.d_loss, var_list=d_vars)
            self.d_opt = optimizers.dp_optimizer.DPAdamGaussianOptimizer(
                learning_rate=self.d_learning_rate,
                name='D_opt',
                l2_norm_clip = 10,
                noise_multiplier = 0.00005,
                num_microbatches = int(self.batch_size/4),
                beta1=self.beta1,
                beta2=self.beta2)

            D_grads_and_vars = self.d_opt.compute_gradients(loss=self.d_loss, var_list=d_vars)
            self.d_optim = self.d_opt.apply_gradients(D_grads_and_vars)

            self.g_opt = MovingAverageOptimizer(tf.train.AdamOptimizer(self.g_learning_rate, beta1=self.beta1, beta2=self.beta2), average_decay=self.moving_decay)
            self.g_optim = self.g_opt.minimize(self.g_loss, var_list=g_vars)

        """" Testing """
        # for test
        self.fake_images = self.generator(self.z, is_training=False, reuse=True)
        self.fake_images_random_normal = self.generator(tf.random_normal(shape=[self.batch_size, 1, 1, self.z_dim], name='random_z_latents'), is_training=False, reuse=True)

        # ppl_zfull graph
        lat_t01 = tf.random_normal(shape=[16, 128])
        lerp_t = tf.random_uniform([8], 0.0, 1.0)
        lat_t0, lat_t1 = lat_t01[0::2], lat_t01[1::2]
        lat_e0 = slerp(lat_t0, lat_t1, lerp_t[:, np.newaxis])
        lat_e1 = slerp(lat_t0, lat_t1, lerp_t[:, np.newaxis] + 1e-4)
        lat_e01 = tf.reshape(tf.stack([lat_e0, lat_e1], axis=1), lat_t01.shape)
        lat_e01 = tf.expand_dims(lat_e01, axis=1)
        lat_e01 = tf.expand_dims(lat_e01, axis=1)
        self.interp_images = self.generator(lat_e01, is_training=False, reuse=True)


        """ Summary """
        self.d_sum = tf.summary.scalar("d_loss", tf.reduce_mean(self.d_loss))
    ...
    def train(self):
        ...
        # saver to save model
        self.saver = self.g_opt.swapping_saver(max_to_keep=0)
        ...
        if self.checkpoint_name != None:
            ...
            could_load, checkpoint_counter = self.load(self.checkpoint_name)
        ...
        if not os.path.exists(self.base_results_dir + self.results_dir):
            os.makedirs(self.base_results_dir + self.results_dir)
        else:
            sub_dirs = [dI for dI in sorted(os.listdir(self.base_results_dir)) if os.path.isdir(os.path.join(self.base_results_dir,dI))]
            last = int(sub_dirs[-1])
            self.results_dir = '{:04d}'.format(last + 1)
            os.mkdir(self.base_results_dir + self.results_dir)
            print('dir ' + self.results_dir + ' created')
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.iteration):
                ...
                # save training results for every tot steps
                if np.mod(idx + 1, self.print_freq) == 0:
                    ...
                    save_images(samples[:manifold_h * manifold_w, :, :, :],
                                ...
                                self.base_results_dir + self.results_dir + '/' + self.model_name + '_train_{:02d}_{:05d}.png'.format(
                                    epoch, idx + 1))
            ...
            print("Epoch: [%2d] time: %4.4f, d_loss: %.8f, g_loss: %.8f" \
                  % (epoch, time.time() - start_time, d_loss, g_loss))
